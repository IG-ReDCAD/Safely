    ...
    if(exist_route is None):
        ...
        for n in list_neigh:
            ...
            if(neigh is not None):
                print(new_route.route_id, neigh.neigh_id)
                # add the neigh route 
...
def send_m():
    ...
    print(account_sid,auth_token, my_phone, user.phone_num)
...
def share_link():
    ...
    # short url
    # API_Key = 'AIzaSyCS8bnj78vA2fuZ7dBPdNBgHmFuCbvpuus'
...
def update_neigh():
    ...
    if len(number_names)==39:
        for index in range(len(number_names)):
            ...
            print(neigh.neigh_id)
            print("old name", name[index],"new name", neigh.neigh_name)
